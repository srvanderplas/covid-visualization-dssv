...
\begin{figure}
\includegraphics[width=.99\linewidth]{covid-tracking-hex-cartogram}
...
\begin{figure}
\includegraphics[width=.99\linewidth]{nyt-covid-scaled-choro}
...
% Is it worth discussing e.g. https://www.nytimes.com/interactive/2021/us/covid-risk-map.html that does not make this distinction and runs into the "boy who cried wolf" problem? When everything is always extremely high risk we just tend to tune things out. I don't want to distract from the overview, though...
...
Visualisations are typically created for a specific purpose, and during a pandemic it is particularly critical to show the evolution of the situation over time. This not only allows individuals to compare the present to past case counts, but also allows for forecasting and prediction of the future based on the current state. Time series plots are the most natural choice for these goals, and over the course of the pandemic, many different attempts have been made to show time series information using different graphical forms. 
...
ggplot(data = tmp2, aes(x = date, ymin = 0, ymax = y, 
  ...
  scale_colour_gradientn(guide = 'none', paste0("Days to ", n/1000, "k\nmore cases"), space = "Lab", 
    trans = "log10",
