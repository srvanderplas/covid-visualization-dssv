...
p1 <-  tmp %>%
  ...
  geom_textline(aes(color = state, label = state, vjust = -0.1, hjust =.93) , text_smoothing = 50) + 
...
p2 <-  tmp %>%
  ggplot(aes(x = date, y = daily_avg_cases, group = state)) +
  geom_textline(aes(color = state, label = state, vjust = -0.25, hjust = .85) , text_smoothing = 60) + 
  ylab("Daily average cases (7 day average)") + 
  xlab("Date") + 
  guides(color = "none") + 
  facet_grid(~"Linear") + 
...
<<log-scales-ref-lines, fig.width = 8, fig.height = 4, out.width = ".8\\linewidth", fig.cap = "Reported COVID cases in New York State, 2020-2022. The linear scale makes it difficult to compare the trajectory of different waves to determine how severe the current status is relative to the past.">>=
cvstate %>%
  filter(state == "New York") %>%
  ggplot(aes(x = date, y = daily_avg_cases)) + 
  geom_line() + 
