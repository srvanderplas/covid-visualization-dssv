...
author:
  - name: Patrick J.F. Groenen
    ...
    email: \email{groenen@ese.eur.nl}
  ...
  - name: Stefan Van Aelst
    ...
    address: |
      ...
      |  3001 Leuven, Belgium
    email: \email{Stefan.VanAelst@kuleuven.be}
...
title:
  formatted: "Guidelines for the Journal of Data Science, Statistics, and Visualisation"
  # If you use tex in the formatted title, also supply version without
  plain:     "Guidelines for the Journal of Data Science, Statistics, and Visualisation Version 0.3"
  # For running headers, if needed
  short:     "JDSSV Guidelines, Version 0.3"
abstract: >
...
keywords:
  ...
  formatted: [JDSSV, style guidelines, comma-separated, not capitalized, "\\proglang{R}"]
  plain:     [JDSSV, style guidelines, comma-separated, not capitalized, R]
preamble: >
  \usepackage{amsmath}
...
[comment]: # (\todo{\tiny Alternatives: (1) We store the data/code on our JDSSV website with the article. (2) We upload it on Zenodo ourselves (not sure if that can be done).)  
...
# Correspondence Analysis {#sec:models}
...
\end{eqnarray*}
is minimized \citep[see, for example,][]{vandeveldenetal2009seriation}. It may be verified that the least-squares optimal solution is obtained by computing the singular value decomposition (SVD) 
\begin{eqnarray*}
...
```{r plot_corana, dependson = "corana", echo = TRUE}
...
```{r ca_biplot_dims23, dependson = "plot_corana", echo = TRUE, fig.width = 6, fig.height = 6, out.width = "10cm", fig.align = "center", fig.cap = "Biplot of a correspondence analysis on word counts used in descriptions of data science by different web sources in dimensions 2 and 3."}
load("ds.word.cnt.RData")
source("compute.pos.R")
...
* Specific markup (\verb|\proglang|, \verb|\pkg|, \verb|\code|) should be used in the references.
* Titles should be inserted in title case.
* Journal titles should not be abbreviated and in title case.
* DOIs should be included where available.
* Software should be properly cited as well. For \proglang{R} packages \code{citation("pkgname")} typically provides a good starting point.
